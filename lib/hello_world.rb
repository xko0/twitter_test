...
def hello_world
  ...
  journalists_random = journalists.sample 5
  journalists_random.each do |tweet|
    client.update("#{tweet} #Hello_world Hey, nice job so far! @the_hacking_pro")
  end
end
=end
=begin
def like_tweets
  client.search("#Hello_world", result_type: "recent").take(30).each do |tweet|
...
def follow_user
  ...
  client.search("#bonjour_monde" result_type: "recent").take(20).each do |tweet|
    unless "#{tweet.user.screen_name}" == "xko0Rgb"
